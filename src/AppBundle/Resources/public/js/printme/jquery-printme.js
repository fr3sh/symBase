...
jQuery.fn.printMe = function (options) {
    ...
    return this.each(function () {
       ...
       tmp = Array.isArray(settings.path);
            if (tmp) {
                 if (settings.path != "") {
            ...
            {
                var arr = $.map(settings.path, function(el) { return el });
             //   var arr = $.makeArray( settings.path );
              
                count = arr.length;
                vR  = arr[0];
                  for (i = 0; i < count; i++) {
                      objPrint.write("<link href='" + arr[i] + "' rel='stylesheet'>");
                  }
